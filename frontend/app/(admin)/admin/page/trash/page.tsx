...
import PageTrashList from "./PageTrashList";
import { serverGet } from "@/app/lib/serverFetch";

export const metadata = {
  title: "سطل زباله صفحات",
  description: "لیست صفحات دور ریخته شده",
};
async function PagePage() {
  let pages;

  try {
    pages = await serverGet(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/content/page/trash`,
    );
  } catch (error) {
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted">مدیریت صفحه‌های آگهی‌ها</p>
            </div>
          </div>
        </div>
      ...
      <Suspense fallback={null}>
        <SuccessMessage
          messages={{
            ...
            editSuccess: "صفحه با موفقیت ویرایش شد.",
            deleteSuccess: "صفحه با موفقیت حذف موقت شد.",
            restoreSuccess: "صفحه با موفقیت بازگردنده شد.",
            restoresSuccess: "صفحه با موفقیت بازگردنده شدند.",
            forceDeleteSuccess: "صفحه با موفقیت حذف دائم شدند.",
          }}
        />
      </Suspense>

      {/* ================= CATEGORY LIST ================= */}
      <PageTrashList pages={pages} />
    </div>
  );
}
