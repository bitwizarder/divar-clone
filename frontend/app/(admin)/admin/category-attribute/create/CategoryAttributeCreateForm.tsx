...
function CategoryAttributeCreateForm() {
  // ========== State‌های مربوط به ویژگی ==========
  const [name, setName] = useState("");
  const [unit, setUnit] = useState(""); // واحد اندازه‌گیری
  const [type, setType] = useState<0 | 1>(0); // 0 = عادی, 1 = ویژه
  const [status, setStatus] = useState(1);
  const [categoryId, setCategoryId] = useState<number | null>(null); // شناسه دسته‌بندی والد
  ...
  useTimeout(
    () => {
      if (success) {
        router.push("/admin/category-attribute?createSuccess=1");
      }
    },
    success ? 800 : null,
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 422) {
          const result = await res.json();
          // ساده‌ترین راه: بگو که errors یک آبجکت از آرایه‌های رشته‌ای است
          const errors = result.errors as Record<string, string[]> | undefined;
          // استخراج اولین پیام خطا
          const firstErrorMessage = errors
            ? Object.values(errors)[0]?.[0]
            : null;
          // اگر خطای اختصاصی داشت که همان را می‌ندازیم، وگرنه همان پیام عمومی را می‌ندازیم
          throw new Error(
            firstErrorMessage ||
              result.message ||
        ...
        throw new Error(
          result.message || "خطایی در ایجاد ویژگی دسته‌بندی رخ داد.",
        );
      }

      setSuccess("ویژگی دسته‌بندی با موفقیت ایجاد شد.");
      ...
      <div className="space-y-6">
        <div className="mb-5">
          ...
          <p className="mt-1 text-sm text-gray">
            اطلاعات اصلی ویژگی دسته‌بندی را وارد کنید.
          </p>
        </div>
