...
function StateList({ states }: { states: StateListResponse }) {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-color bg-surface px-6 py-12 text-center">
        ...
        <p className="mt-2 text-sm text-secondary">
          هنوز هیچ منطقه‌ای برای نمایش ثبت نشده است.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <BulkDeleteAction
        ...
        endpoint="/api/admin/advertise/state/bulk"
        title="حذف مناطق"
        itemName={`${selectedIds.length} منطقه`}
        description={`آیا از حذف ${selectedIds.length} منطقه انتخاب‌شده مطمئن هستید؟`}
        warning="این عملیات قابل بازگشت نیست. در صورت وجود زیر‌منطقه، آن‌ها نیز ممکن است به‌صورت زنجیره‌ای حذف شوند."
        ...
        onSuccess={() => {
          setSelectedIds([]);
          setError(null);
          router.push("/admin/state?deletesSuccess=1");
          router.refresh();
        }}
        onError={(message) => {
          setError(message);
          ...
          <table className="w-full min-w-225 text-right">
            <thead>
              <tr className="border-b border-color bg-surface">
                ...
                <th className="px-3 py-4 text-xs font-semibold">نام منطقه</th>

                <th className="px-3 py-4 text-xs font-semibold">منطقه والد</th>

                <th className="px-3 py-4 text-xs font-semibold">شهر</th>

                <th className="px-3 py-4 text-xs font-semibold">توضیحات</th>
                  ...
                  >
                    ...
                    <td className="px-3 py-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-hover text-icon">
                          <i className={state.icon || "fa fa-flag"}></i>
                        </div>
                    ...
                    <td className="px-3 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <SingleDeleteAction
                          endpoint={`/api/admin/advertise/state/${state.id}`}
                          title="حذف منطقه"
                          itemName={state.name}
                          itemId={state.id}
                          icon={state.icon || "fa fa-flag"}
                          ...
                          onSuccess={() => {
                            ...
                            router.push("/admin/state?deleteSuccess=1");
                            router.refresh();
                          }}
                          onError={(message) => {
                            setError(message);
          ...
          return (
            ...
            >
              ...
              <div className="mt-3 rounded-xl bg-muted p-3">
                <p className="text-xs text-muted">توضیحات</p>

                <p className="mt-1 text-sm leading-6 text-secondary line-clamp-3">
                  {state.description || "توضیحاتی ثبت نشده است."}
                </p>
              </div>
              ...
              <div className="mt-4 flex gap-2 border-t border-color pt-4">
                ...
                <SingleDeleteAction
                  ...
                  extraClasses="flex-1"
                  description="آیا از حذف این منطقه مطمئن هستید؟"
                  warning="این عملیات قابل بازگشت می باشد. پس از حذف، این منطقه به سطل زباله خواهد رفت."
                  deleteButtonText="حذف"
                  confirmText="بله، حذف شود"
                  cancelText="انصراف"
                  onSuccess={() => {
