...
function CategoryEditForm({ category, categories }: CategoryEditFormProps) {
  ...
  useTimeout(
    () => {
      if (success) {
        router.push("/admin/category?editSuccess=1");
      }
    },
    success ? 800 : null,
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/advertise/category/${category.id}`,
        {
          method: "Put",
          headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(result.message || "خطایی در ویرایش دسته‌بندی رخ داد.");
      }
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div>
              ...
              <div className="relative">
                <i
                  className={`${icon || category.icon || "fa fa-image"} absolute right-3 top-1/2 -translate-y-1/2 text-gray-400`}
                ></i>
        ...
        <div className="flex flex-col-reverse gap-3 border-t border-color pt-6 sm:flex-row sm:justify-start">
          <button
            ...
            dark:disabled:bg-gray-300/50 disabled:opacity-100"
          >
            {loading ? (
              <>
