...
function MenuTrashList({ menus }: { menus: MenuListResponse }) {
  ...
  return (
    <>
      ...
      <BulkRestoreAction
        ...
        endpoint="/api/admin/content/menu/trash/restore"
        title="بازیابی منوها"
        itemName={`${selectedIds.length} منو`}
        description={`آیا از بازیابی ${selectedIds.length} منو انتخاب‌شده مطمئن هستید؟`}
        warning="منوهای انتخاب‌شده از سطل زباله خارج و به لیست اصلی بازگردانده می‌شوند."
        onSuccess={() => {
          setSelectedIds([]);
          setError(null);

          router.push("/admin/menu/trash/?restoresSuccess=1");
        }}
        onError={(message) => {
          setError(message);
        }}
      ...
      <BulkDeleteAction
        selectedIds={selectedIds}
        endpoint="/api/admin/content/menu/trash"
        title="حذف دائمی منو‌ها"
        itemName={`${selectedIds.length} منو`}
        description={`آیا از حذف دائمی ${selectedIds.length} منو انتخاب‌شده مطمئن هستید؟`}
        warning="هشدار: این عملیات غیرقابل بازگشت است و منو‌ها برای همیشه از پایگاه داده حذف خواهند شد."
                  ...
                  >
                    ...
                    <td className="px-3 py-4">
                      <div className="flex items-center gap-2">
                        <div className="flex shrink-0 items-center rounded-xl bg-hover text-icon">
                    ...
                    <td className="px-3 py-4">
                      <div className="flex items-center gap-3">
                        <span className=" text-primary">{menu.position}</span>
                      </div>
                    </td>
                    <td className="px-3 py-4">
                      <div className="flex items-center gap-3">
                        <span className=" text-primary">{menu.url}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-4 text-sm text-secondary">
                      {converterToJalali(menu.deleted_at)}
                    </td>

                    <td className="px-3 py-4">
                      <div className="flex items-center gap-2">
                        <SingleRestoreAction
                          endpoint={`/api/admin/content/menu/trash/${menu.id}/restore`}
                          title="بازیابی منو"
                          itemName={menu.title}
                          itemId={menu.id}
                          ...
                          onSuccess={() => {
                            setError(null);
                            router.push("/admin/menu/trash/?restoreSuccess=1");
                          }}
                          onError={(message) => {
                            setError(message);
                          }}
                        ...
                        <SingleDeleteAction
                          endpoint={`/api/admin/content/menu/trash/${menu.id}`}
                          title="حذف دائمی منو"
                          itemName={menu.title}
                          itemId={menu.id}
                          icon="fa fa-trash"
                          description="آیا از حذف دائمی این منو مطمئن هستید؟"
                          warning="هشدار: این عملیات غیرقابل بازگشت است و منو برای همیشه از پایگاه داده حذف خواهد شد."
                          ...
                          onSuccess={() => {
                            ...
                            router.push(
                              "/admin/menu/trash/?forceDeleteSuccess=1",
                            );
                            router.refresh();
                          }}
                          onError={(message) => {
