...
import { MenuCreateInput } from "@/app/types/menu";
...
function MenuCreateForm() {
  ...
  const [menus, setMenus] = useState<MenuCreateInput[]>([]);
  ...
  useEffect(() => {
    const fetchMenus = async () => {
      try {
        ...
        const message =
          error instanceof Error ? error.message : "خطا در دریافت منوها";
  ...
  const validateForm = () => {
    ...
    if (position.trim().length < 2 || position.trim().length > 250) {
      setError("موقعیت باید بین ۲ تا ۲۵۰ کاراکتر باشد.");
      return false;
    }
  ...
  useTimeout(
    () => {
      if (success) {
        router.push("/admin/menu?createSuccess=1");
      }
    },
    success ? 800 : null,
  );
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(result.message || "خطایی در ایجاد منو رخ داد.");
      }
