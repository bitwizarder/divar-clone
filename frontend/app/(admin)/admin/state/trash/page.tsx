...
import StateTrashList from "./StateTrashList";
import { serverGet } from "@/app/lib/serverFetch";

export const metadata = {
  title: "سطل زباله مناطق",
  description: "لیست مناطق دور ریخته شده",
};
...
async function StatePage() {
  ...
  try {
    states = await serverGet(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/advertise/state/trash`,
    );
  } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Suspense fallback={null}>
        <SuccessMessage
          messages={{
            createSuccess: "منطقه با موفقیت ایجاد شد.",
            editSuccess: "منطقه با موفقیت ویرایش شد.",
            deleteSuccess: "منطقه با موفقیت حذف موقت شد.",
            restoreSuccess: "منطقه با موفقیت بازگردنده شد.",
            restoresSuccess: "منطقه با موفقیت بازگردنده شدند.",
            forceDeleteSuccess: "منطقه با موفقیت حذف دائم شدند.",
          }}
        />
      </Suspense>

      {/* ================= CATEGORY LIST ================= */}
      <StateTrashList states={states} />
    </div>
  );
}
