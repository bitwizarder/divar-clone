...
import { StateEditFormProps } from "@/app/types/state";
import { useTimeout } from "@/app/hooks/useTimeout";
import { City } from "@/app/types/city";

function StateEditForm({ state, states }: StateEditFormProps) {
  const [name, setName] = useState(state.name);
  const [description, setDescription] = useState(state.description);
  const [status, setStatus] = useState(state.status);
  const [icon, setIcon] = useState<string | null>(state.icon || "");
  const [parentId, setParentId] = useState<number | null>(
    state.parent_id ?? null,
  );
  const [cityId, setCityId] = useState<number | null>(state.city?.id ?? null);
  const [selectedParentCityId, setSelectedParentCityId] = useState<
    number | null
  >(null);
  ...
  const validateForm = () => {
    ...
    if (parentId && !cityId) {
      setError(
        "والد انتخاب‌شده فاقد شهر است. لطفاً شهر را به‌صورت دستی انتخاب کنید یا والد دیگری انتخاب کنید.",
      );
      return false;
    }
    if (!parentId && !cityId) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/advertise/state/${state.id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(result.message || "خطایی در ویرایش منطقه رخ داد.");
      }
  ...
  useTimeout(
    () => {
      if (success) {
        router.push("/admin/state?editSuccess=1");
      }
    },
    success ? 800 : null,
  );
  ...
  return (
    <div className="space-y-6">
      {/* خطاها و موفقیت */}
      {error && (
        <div className="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 p-4 text-red-700 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400">
      ...
      {success && (
        <div className="flex items-start gap-3 rounded-xl border border-green-200 bg-green-50 p-4 text-green-700 dark:border-green-800 dark:bg-green-900/20 dark:text-green-400">
        ...
        <div>
          <div className="mb-5">
            ...
            <p className="mt-1 text-sm text-gray">
              اطلاعات اصلی منطقه را وارد کنید.
            </p>
          </div>
            ...
            <div>
              ...
              {parentId && selectedParentCityId && (
                <p className="mt-1 text-xs text-blue-600">
                  شهر از والد ({states.find((s) => s.id === parentId)?.name})
                  گرفته شده است.
                </p>
              )}
              {parentId && !selectedParentCityId && (
                <p className="mt-1 text-xs text-orange-500">
                  والد انتخاب‌شده فاقد شهر است. لطفاً شهر را به‌صورت دستی انتخاب
                  کنید.
                </p>
              )}
            </div>
        ...
        <div className="flex flex-col-reverse gap-3 border-t border-color pt-6 sm:flex-row sm:justify-start">
          ...
          >
            {loading ? (
              ...
              <>
                <i className="fa-solid fa-pen-to-square"></i> ویرایش
              </>
            )}
          </button>
        </div>
