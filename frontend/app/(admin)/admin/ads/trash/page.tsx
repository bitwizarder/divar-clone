...
import AdsTrashList from "./AdsTrashList";
import { serverGet } from "@/app/lib/serverFetch";
export const metadata = {
  title: "سطل زباله آگهی‌ها",
  description: "لیست آگهی‌ها‌ی دور ریخته شده",
};
async function AdsPage() {
  let adses;

  try {
    adses = await serverGet(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/advertise/advertisement/trash`,
    );
  } catch (error) {
    console.error("Error fetching adses:", error);
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted">مدیریت آگهی‌ها</p>
            </div>
          </div>
        </div>
      ...
      <Suspense fallback={null}>
        <SuccessMessage
          messages={{
            createSuccess: "آگهی با موفقیت ایجاد شد.",
            editSuccess: "آگهی با موفقیت ویرایش شد.",
            deleteSuccess: "آگهی با موفقیت حذف موقت شد.",
            restoreSuccess: "آگهی با موفقیت بازگردنده شد.",
            restoresSuccess: "آگهی با موفقیت بازگردنده شدند.",
            forceDeleteSuccess: "آگهی با موفقیت حذف دائم شدند.",
          }}
        />
      </Suspense>

      {/* ================= CATEGORY LIST ================= */}
      <AdsTrashList adses={adses} />
    </div>
  );
}
