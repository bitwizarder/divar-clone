...
import { CategoryListResponse } from "@/app/types/category";
import { converterToJalali } from "@/app/helpers/date";

function CategoryList({ categories }: { categories: CategoryListResponse }) {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-color bg-surface px-6 py-12 text-center">
        ...
        <p className="mt-2 text-sm text-secondary">
          هنوز هیچ دسته‌بندی‌ای برای نمایش ثبت نشده است.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <BulkDeleteAction
        ...
        endpoint="/api/admin/advertise/category/bulk"
        title="حذف دسته‌بندی‌ها"
        itemName={`${selectedIds.length} دسته‌بندی`}
        description={`آیا از حذف ${selectedIds.length} دسته‌بندی انتخاب‌شده مطمئن هستید؟`}
        warning="این عملیات قابل بازگشت نیست. در صورت وجود زیر‌دسته، آن‌ها نیز ممکن است به‌صورت زنجیره‌ای حذف شوند."
        ...
        onSuccess={() => {
          setSelectedIds([]);
          setError(null);
          router.push("/admin/category?deletesSuccess=1");
          router.refresh();
        }}
        onError={(message) => {
          setError(message);
              ...
              <tr className="border-b border-color bg-surface">
                ...
                <th className="w-14 px-3 py-4">
                  <input
                    ...
                    checked={allSelected}
                    onChange={toggleAll}
                    aria-label="انتخاب همه دسته‌بندی‌ها"
                ...
                <th className="px-3 py-4 text-xs font-semibold">دسته والد</th>
                  ...
                  >
                    ...
                    <td className="px-3 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <SingleDeleteAction
                          endpoint={`/api/admin/advertise/category/${category.id}`}
                          title="حذف دسته‌بندی"
                          ...
                          onSuccess={() => {
                            ...
                            router.push("/admin/category?deleteSuccess=1");
                            router.refresh();
                          }}
                          onError={(message) => {
                            setError(message);
        ...
        <div className="flex items-center justify-between rounded-2xl border border-color bg-surface px-4 py-3">
          ...
          <span className="text-xs text-muted">
            {selectedIds.length > 0
              ...
              : `${categories.data.length} دسته‌بندی`}
          </span>
        </div>
            ...
            >
              ...
              <div className="mt-3 rounded-xl bg-muted p-3">
                ...
                <p className="mt-1 text-sm line-clamp-3 leading-6 text-secondary">
                  {category.description || "توضیحاتی ثبت نشده است."}
                </p>
              </div>
              ...
              <div className="mt-4 flex gap-2 border-t border-color pt-4">
                ...
                <SingleDeleteAction
                  ...
                  extraClasses="flex-1"
                  description="آیا از حذف این دسته‌بندی مطمئن هستید؟"
                  warning="این عملیات قابل بازگشت می باشد. پس از حذف، این دسته‌بندی به سطل زباله خواهد رفت."
                  deleteButtonText="حذف"
                  confirmText="بله، حذف شود"
                  cancelText="انصراف"
                  onSuccess={() => {
