...
interface CategoryValueEditFormProps {
  value: CategoryValue;
  attributes: { id: number; name: string; unit: string }[];
}

function CategoryValueEditForm({
  value,
  attributes,
}: CategoryValueEditFormProps) {
  // ========== State‌های مربوط به مقدار ==========
  const [val, setVal] = useState(value.value);
  const [type, setType] = useState<0 | 1>(value.type as 0 | 1);
  const [status, setStatus] = useState(value.status);
  const [attributeId, setAttributeId] = useState<number | null>(
    value.categoryAttribute?.id ?? null,
  );
  ...
  useTimeout(
    () => {
      if (success) {
        router.push("/admin/category-value?editSuccess=1");
      }
    },
    success ? 800 : null
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/advertise/category-value/${value.id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!res.ok) {
        ...
        if (res.status === 422) {
          ...
          const firstErrorMessage = errors ? Object.values(errors)[0]?.[0] : null;
          throw new Error(
            firstErrorMessage || result.message || "اطلاعات وارد شده معتبر نیست."
          );
        }
        throw new Error(result.message || "خطایی در ویرایش مقدار ویژگی رخ داد.");
      }

      setSuccess("مقدار ویژگی با موفقیت ویرایش شد.");
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        <div className="mb-5">
          <h2 className="text-lg font-bold text-secondary">اطلاعات مقدار ویژگی</h2>
          <p className="mt-1 text-sm text-gray">اطلاعات مقدار ویژگی را ویرایش کنید.</p>
        </div>

        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div>
            <label htmlFor="val" className="mb-2 block text-sm font-medium text-gray-dark">
              مقدار
            </label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="مثلاً ۴۱۰۰۰ یا قرمز"
          ...
          <div>
            <label htmlFor="attribute_id" className="mb-2 block text-sm font-medium text-gray-dark">
              ویژگی
            </label>
            <select
              id="attribute_id"
              value={attributeId ?? ""}
              onChange={(e) => setAttributeId(e.target.value ? Number(e.target.value) : null)}
            ...
            <p className="mt-2 text-xs text-gray-400">ویژگی‌ای که مقدار به آن تعلق دارد</p>
          </div>

          <div>
            <label htmlFor="type" className="mb-2 block text-sm font-medium text-gray-dark">
              نوع مقدار
            </label>
            <select
              id="type"
