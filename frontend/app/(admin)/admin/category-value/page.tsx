...
import StateList from "./CategoryValueList";
import { serverGet } from "@/app/lib/serverFetch";
export const metadata = {
  title: "پنل مدیریت | مقادیر دسته‌ها",
  description: "لیست مقادیر دسته‌ها در پنل مدیریت",
};
async function StatePage() {
  ...
  try {
    ...
    console.error("Error fetching categoryValues:", error);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-color bg-surface p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted flex items-center flex-wrap gap-2">
                <span>مدیریت مقادیر دسته‌های آگهی‌ها</span>
                {categoryValues.categoryValue_count && (
                  <span className="bg-hover px-3 py-1 text-gray rounded-full">
                    تعداد رکورد ها ({categoryValues.categoryValue_count || 0})
                  </span>
                )}
              </p>
            </div>
        ...
        <div className="flex gap-2">
          <Link
            href="/admin/category-value/trash"
          ...
          >
            ...
            <span className="text-nowrap">ایجاد مقادیر دسته جدید</span>
          </Link>
        </div>
      </div>
      ...
      <Suspense fallback={null}>
        <SuccessMessage
          messages={{
            createSuccess: "مقادیر دسته با موفقیت ایجاد شد.",
            editSuccess: "مقادیر دسته با موفقیت ویرایش شد.",
            deleteSuccess: "مقادیر دسته با موفقیت حذف شد.",
            deletesSuccess: "مقادیر دسته های انتخاب شده با موفقیت حذف شدند.",
          }}
        />
      </Suspense>

      {/* ================= STATE LIST ================= */}
      <StateList values={categoryValues} />
    </div>
  );
}
